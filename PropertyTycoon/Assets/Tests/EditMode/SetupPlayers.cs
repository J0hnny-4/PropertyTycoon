...
namespace Tests.EditMode
{
    public class SetupPlayers
    {
        ...
        public void Setup()
        {
            ...
            _controller = new PlayerSetupController(MinPlayers, MaxPlayers, _names);
        ...
        {
            Assert.AreEqual(MinPlayers, _controller.PlayersCount, "Initialised to wrong player count.");
            var tokens = _controller.GetAllTokens();
            Assert.IsNotNull(tokens, "Cannot get tokens from resource.");
            Assert.IsTrue(tokens.Length >= MaxPlayers, "The tokens available are less than the max number of players.");
        }
        ...
        public void TestAddPlayer()
        {
            ...
            Assert.AreEqual(MinPlayers + 1, _controller.PlayersCount, "Players count is incorrect.");
        ...
        public void TestRemovePlayer()
        {
            ...
            Assert.AreEqual( MinPlayers, _controller.PlayersCount, "Players count is incorrect.");
            Assert.IsTrue(eventFired, "Event was not fired.");
        ...
        {
            // currently already at minimum players required
            for (var i = MinPlayers; i < MaxPlayers; i++) { _controller.AddPlayer(); }
        ...
        {
            if (_controller.GetAllTokens().Length < MinPlayers + 2)
            {
        ...
        public void TestCannotSwitchTokenWhenNoMoreAvailable()
        {
            // use all tokens
            var tokens = _controller.GetAllTokens();
            for (var i = MinPlayers; i < tokens.Length; i++) { _controller.GetNextAvailableToken(); }
